...
class FleetEnv(gym.Env):
    ...
    def __init__(
        ...
        per_ac_dims = (
            [self.masker.n_bases]
            + [self.masker.n_wpn] * NUM_HARDPOINTS
            + [self.masker.n_eqp]
            + [self.masker.n_msn]
        )
    ...
    def step(self, action: np.ndarray) -> tuple[dict, float, bool, bool, dict]:
        ...
        for i, ac_id in enumerate(ac_ids):
            ...
            parsed[ac_id] = {
                "base_idx": int(a[0]),
                "wpn": [int(a[1 + h]) for h in range(NUM_HARDPOINTS)],
                "eqp": int(a[1 + NUM_HARDPOINTS]),
                "msn": int(a[2 + NUM_HARDPOINTS]),
            }
        ...
        for ac_id in ac_ids:
            ...
            if base is not None:
                for hp in range(NUM_HARDPOINTS):
                    wpn_act = act["wpn"][hp]
                    current = int(ac.weapons[hp])
                    if wpn_act == 0:
                        if current > 0:
                            base.weapons[current] = base.weapons.get(current, 0) + 1
                            ac.weapons[hp] = 0.0
                    else:
                        new_wid = self.masker.weapon_ids[wpn_act - 1]
                        if new_wid != current:
                            if current > 0:
                                base.weapons[current] = base.weapons.get(current, 0) + 1
                            if base.weapons.get(new_wid, 0) > 0:
                                base.weapons[new_wid] -= 1
                                ac.weapons[hp] = float(new_wid)
