...
def _build_aircraft_form(game: FleetGame, ac_id: int) -> dict:
    ...
    if ac.status != AircraftStatus.AVAILABLE:
        return {
            ...
            "defaults": {
                ...
                "detailedBaseRaw": 0,
                "detailedWeaponRaws": [0] * NUM_HARDPOINTS,
    ...
    base_mask = masks[0]
    weapon_masks = masks[1:1 + NUM_HARDPOINTS]
    equipment_mask = masks[1 + NUM_HARDPOINTS]
    mission_mask = masks[2 + NUM_HARDPOINTS]
    ...
    for hardpoint_idx in range(NUM_HARDPOINTS):
        valid: List[dict] = []
        default_index = 0
        current_weapon = int(ac.weapons[hardpoint_idx])
        for weapon_idx in range(len(weapon_masks[hardpoint_idx])):
            if not weapon_masks[hardpoint_idx][weapon_idx]:
                continue
            if weapon_idx == 0:
                label = "Empty (unload)"
                weapon_id = 0
            else:
                weapon_id = masker.weapon_ids[weapon_idx - 1]
                label = game.weapon_names.get(weapon_id, f"Weapon {weapon_id}")
                if int(weapon_id) == current_weapon:
                    label += " (keep)"
                    default_index = len(valid)
            valid.append(_choice(weapon_idx, label, weaponId=int(weapon_id)))
        weapon_options.append(valid)
        weapon_default_raws.append(
            int(
                _pick_raw(
                    valid,
                    suggested_action[1 + hardpoint_idx] if suggested_action else None,
    ...
    return {
        ...
        "defaults": {
            ...
            "detailedEquipmentRaw": int(
                _pick_raw(
                    equipment_options,
                    suggested_action[1 + NUM_HARDPOINTS] if suggested_action else None,
                    fallback_index=0,
                )
                or 0
            ),
    ...
    for aircraft_id in sorted(game.env.fleet):
        ...
        if mode == 2:
            ...
            if base_raw not in valid:
                full_action.extend(game._default_action(aircraft_id))
                continue
            full_action.extend([int(base_raw)] + [0] * NUM_HARDPOINTS + [0] + [0])
            continue
        ...
        for hardpoint_idx in range(NUM_HARDPOINTS):
            valid = {
                option["raw"]
                for option in form["detailed"]["weaponOptions"][hardpoint_idx]
            }
            desired_raw = None
            if hardpoint_idx < len(incoming_weapons):
                desired_raw = _as_int(incoming_weapons[hardpoint_idx], None)
            if desired_raw not in valid:
                desired_raw = int(default_weapon_raws[hardpoint_idx])
