...
MultiDiscrete space defined in ``FleetEnv``::
    ...
    [target_base, wpn_1 … wpn_6, equipment, mission]
...
from aircraft import Aircraft, AircraftBounds, AircraftStatus, NUM_HARDPOINTS
...
class ActionMasker:
    ...
    def __init__(
        ...
        self._wpn_to_idx: Dict[int, int] = {
            wid: i + 1 for i, wid in enumerate(self.weapon_ids)
        }
    ...
    def _weapon_mask(
        ...
        """Mask for one hardpoint (1-indexed).

        A weapon is valid when the hardpoint allows it **and** the base
        stocks it (or the weapon is already loaded on this hardpoint).
        """
        mask = np.zeros(self.n_wpn, dtype=bool)
        mask[0] = True                               # empty / unload

        if ac.status != AircraftStatus.AVAILABLE or base is None:
            return mask

        allowed = set(self.bounds.allowed_weapons.get(hardpoint, []))
        current_wpn_id = int(ac.weapons[hardpoint - 1])

        for wid in self.weapon_ids:
            action_idx = self._wpn_to_idx[wid]
            if wid not in allowed:
                continue
            if wid == current_wpn_id:
                mask[action_idx] = True              # already loaded, free
                continue
            if base.weapons.get(wid, 0) > 0:
    ...
    def mask_for_aircraft(
        ...
        ``MultiDiscrete`` space (1 + NUM_HARDPOINTS + 1 + 1 = 9).
        """
        base = bases.get(ac.base_id)

        masks: List[np.ndarray] = [self._base_mask(ac, bases)]
        for hp in range(1, NUM_HARDPOINTS + 1):
            masks.append(self._weapon_mask(ac, base, hp))
