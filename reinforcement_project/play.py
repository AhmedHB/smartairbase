...
from aircraft import AircraftStatus, NUM_HARDPOINTS, NUM_EQUIPMENT_SLOTS
...
class FleetGame:
    ...
    def _infer_mode_from_action(
        ...
        if (
            suggested_action[1:1 + NUM_HARDPOINTS]
            == default_action[1:1 + NUM_HARDPOINTS]
            and suggested_action[1 + NUM_HARDPOINTS] == 0
    ...
    def _describe_ai_plan(self, ac_id: int, suggested_action: Optional[List[int]]) -> str:
        ...
        weapon_changes = sum(
            int(suggested_action[1 + hp] != default_action[1 + hp])
            for hp in range(NUM_HARDPOINTS)
        )
        equipment_change = suggested_action[1 + NUM_HARDPOINTS] != 0
        ...
        if weapon_changes:
            ...
            details.append(f"{weapon_changes} weapon change{suffix}")
        if equipment_change:
    ...
    def _default_action(self, ac_id: int) -> List[int]:
        ...
        for hp in range(NUM_HARDPOINTS):
            wid = int(ac.weapons[hp])
            if wid == 0:
                current_wpns.append(0)
            else:
                idx = self.env.masker._wpn_to_idx.get(wid, 0)
                current_wpns.append(idx)
        return [stay_idx] + current_wpns + [0] + [0]
    ...
    def _get_action_for_aircraft(self, ac_id: int) -> List[int]:
        ...
        base_mask = masks[0]
        wpn_masks = masks[1:1 + NUM_HARDPOINTS]
        eqp_mask = masks[1 + NUM_HARDPOINTS]
        msn_mask = masks[2 + NUM_HARDPOINTS]
    ...
    def _quick_transfer(
        ...
        return [base_idx] + [0] * NUM_HARDPOINTS + [0] + [0]
    ...
    def _detailed(
        ...
        if chosen_bid != ac.base_id:
            action += [0] * NUM_HARDPOINTS + [0] + [0]
            print(c(f"    -> Transferring! Other actions skipped.", YELLOW))
            print()
            return action

        # -- weapons per hardpoint --
        for hp_idx in range(NUM_HARDPOINTS):
            hp_num = hp_idx + 1
            wmask = wpn_masks[hp_idx]
            current_wid = int(ac.weapons[hp_idx])
            current_name = self.weapon_names.get(current_wid, "Empty")
            ...
            for widx in range(len(wmask)):
                ...
                if widx == 0:
                    ...
                    wid = masker.weapon_ids[widx - 1]
                    label = self.weapon_names.get(wid, f"Weapon {wid}")
                    if wid == current_wid:
                        label += " (keep)"
                        default_wpn = len(valid_wpns)
            ...
            print(c(f"  Hardpoint {hp_num} (current: {current_name}):", BOLD))
            for i, (_, label) in enumerate(valid_wpns):
                ai_raw = (
                    suggested_action[1 + hp_idx] if suggested_action is not None else None
                )
        ...
        for eidx in range(len(eqp_mask)):
            ...
            ai_raw = (
                suggested_action[1 + NUM_HARDPOINTS]
                if suggested_action is not None
                else None
            )
            print(
    ...
    def run(self):
        ...
        print(f"    {c('2.', BOLD)} Weapons      (load/unload on 6 hardpoints)")
